      ...
      <main className="overflow-x-hidden bg-paper text-body">
        ...
        <section id="top" className="relative pt-28 pb-14 md:pt-36 md:pb-20">
          ...
          <div className="relative z-10 mx-auto max-w-[1440px] px-6 text-center md:px-12 md:text-left">
            ...
            <h1 className="mt-4 mx-auto font-display text-[clamp(2rem,4.5vw,3.5rem)] font-bold leading-[1.05] tracking-[-0.03em] text-ink max-w-4xl md:mx-0">
            ...
            <FadeUp delay={0.5} play={done}>
              <p className="mt-6 mx-auto max-w-2xl text-sm sm:text-base leading-relaxed text-body font-medium md:mx-0">
        ...
        <section className="mx-auto max-w-[1440px] px-6 py-10 md:px-12">
          <motion.div 
            className="rounded-3xl border border-black/[0.06] dark:border-white/10 bg-surface dark:bg-white/[0.03] p-6 md:p-10 shadow-card backdrop-blur-md flex flex-col items-center text-center md:flex-row md:items-center md:text-left justify-between gap-6"
          ...
          >
            ...
            <div className="max-w-xl space-y-2">
              ...
              <p className="font-display text-sm sm:text-base font-bold text-accent italic">
        ...
        <section className="mx-auto max-w-[1440px] px-6 py-16 md:px-12 md:py-24">
          <div className="grid gap-8 md:grid-cols-2">
            {MEMBERS.map((m, idx) => (
              ...
              >
                <TiltCard className="group flex h-full flex-col rounded-3xl border border-black/[0.06] dark:border-white/10 bg-surface dark:bg-white/[0.03] p-8 shadow-card backdrop-blur-md transition-all duration-500 hover:-translate-y-2 hover:border-accent">
                  {/* Headshot Photo */}
                  <div className="relative aspect-[4/3] w-full overflow-hidden rounded-2xl bg-ink/5 dark:bg-black/40">
                    <Image
                      src={m.image}
                      alt={m.name}
                      fill
                      ...
                      priority={idx < 2}
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-surface via-transparent to-transparent opacity-60 dark:from-[#141419]" />
                  </div>
                  ...
                  <div className="mt-6 flex flex-1 flex-col justify-between">
                    <div>
                      <div className="flex flex-wrap items-center justify-between gap-4">
                        ...
                        <span className="font-mono text-xs uppercase tracking-wider text-accent font-semibold bg-accent/10 px-3 py-1 rounded-full border border-accent/20">
                          {m.role}
                        </span>
                      </div>

                      {/* Favourite Quote (italic) */}
                      <p className="mt-4 italic text-body border-l-2 border-accent pl-4 py-1 leading-relaxed text-sm md:text-base font-medium">
                        &ldquo;{m.quote}&rdquo;
                      </p>

                      {/* Owns Section */}
                      <div className="mt-6 border-t border-black/[0.06] dark:border-white/10 pt-5">
                        <h4 className="font-mono text-xs uppercase tracking-wider text-ink/40 font-semibold mb-3">Owns:</h4>
                        <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2.5">
                          {m.owns.map((point) => (
                            <li key={point} className="flex items-center gap-2.5 text-sm text-ink/90 dark:text-white/90 font-medium">
                              <span className="flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-accent/20 text-accent text-[10px] font-bold">
                                ✓
                              </span>
                              <span>{point}</span>
                            </li>
                    ...
                    {/* LinkedIn button */}
                    {m.linkedin && (
                      <div className="mt-8 flex justify-start border-t border-black/[0.06] dark:border-white/10 pt-4">
                        <a
                          href={m.linkedin}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="inline-flex items-center gap-2 rounded-full border border-black/[0.08] dark:border-white/15 bg-ink/[0.04] dark:bg-white/5 px-5 py-2.5 text-xs font-semibold text-ink transition-all duration-300 hover:border-accent hover:bg-accent hover:text-black"
                        >
        ...
        {/* 4. How We Work */}
        <section className="mx-auto max-w-[1440px] px-6 py-16 md:px-12 md:py-24 border-t border-black/[0.07] dark:border-white/10">
          <div className="grid gap-12 md:grid-cols-5 md:gap-8 items-center">
            <div className="md:col-span-2 text-center md:text-left">
              <FadeUp>
                <Eyebrow index="02" label="How We Work" />
                <h2 className="mt-6 font-display text-4xl font-bold tracking-[-0.03em] text-ink md:text-5xl">
            ...
            <div className="md:col-span-3 text-center md:text-left">
              <FadeUp delay={0.15}>
                <div className="space-y-4 text-lg md:text-xl leading-relaxed text-body font-medium">
                  <p className="font-mono text-sm text-accent font-semibold uppercase tracking-wider">
        ...
        <section className="mx-auto max-w-[1440px] px-6 py-16 md:px-12 md:py-24">
          <motion.div 
            className="rounded-3xl border border-black/[0.06] dark:border-white/10 bg-surface dark:bg-white/[0.03] p-8 md:p-12 shadow-card grid gap-12 md:grid-cols-2"
            initial={{ opacity: 0, y: 32 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, amount: 0.2 }}
            transition={{ duration: 0.8, ease: EASE_OUT_EXPO }}
          >
            <div className="text-center md:text-left">
              ...
              <p className="mt-6 text-base md:text-lg leading-relaxed text-body font-medium">
              ...
              <div className="mt-8 flex justify-center md:justify-start">
                <Link
                  href="/careers"
                  className="inline-flex items-center gap-2 rounded-full bg-accent px-7 py-3.5 text-sm font-semibold text-black shadow-card transition-all duration-300 hover:bg-accent-strong"
                >
            ...
            <div className="flex flex-col justify-center border-t border-black/[0.06] dark:border-white/10 pt-8 text-center md:border-t-0 md:pt-0 md:text-left">
              <h3 className="font-mono text-xs uppercase tracking-wider text-ink/40 font-semibold mb-6">
              ...
              <div className="flex flex-wrap justify-center gap-3 md:justify-start">
                {[
                  ...
                  <span
                    key={role}
                    className="rounded-full border border-black/[0.08] dark:border-white/15 bg-ink/[0.04] dark:bg-white/5 px-5 py-3 text-sm font-semibold text-ink shadow-sm"
                  >
                    {role}
                  </span>
                ))}
